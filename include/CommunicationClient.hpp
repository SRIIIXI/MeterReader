...
{
public:
    CommunicationClient(const unsigned char* isystemTitle,
               int isystemTitlelen,
    ...
    void EnableCommunicationLog(bool fl);
    ...
    void DumpTestHdlcFrames();
    void DumpSetupInformation();
    ...
    void CreatePrePaidRequestFrame(QByteArray& buffer, const QString &obiscode, int index);
    bool CheckPrePaidResponseFrame(const QByteArray& buffer, const QString &obiscode, DLMSAccount &value, bool &isfound, int index);

    void CreateTokenGatewayRequestFrame(QByteArray& buffer, const QString &obiscode);
    bool CheckTokenGatewayResponseFrame(const QByteArray& buffer, const QString &obiscode, DLMSTokenGateway &value, bool &isfound);
    ...
    void CreateAssociationViewRequestFrame(QByteArray& buffer);
    bool CheckAssociationViewResponseFrame();
    ...
    static CommunicationClient* GetClient(const int clientid);
    static void ExtractKeys(DLMSVariant& data, CommunicationClient* client103, CommunicationClient* client104, CommunicationClient* client105);
    //
...
private:
    ...
    DLMSTokenGateway primaryGateway;
